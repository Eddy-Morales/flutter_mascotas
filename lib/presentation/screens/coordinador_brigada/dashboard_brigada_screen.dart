...
class _DashboardBrigadaScreenState extends State<DashboardBrigadaScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final coordId = Provider.of<AuthProvider>(context, listen: false).usuarioActual!.id;
      Provider.of<BrigadaProvider>(context, listen: false).cargarSectorCoordinador(coordId);
      Provider.of<BrigadaProvider>(context, listen: false).cargarVacunadores();
    });
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              if (_cedulaCtrl.text.isEmpty || _nombresCtrl.text.isEmpty || _correoCtrl.text.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Por favor, llene los datos obligatorios.'), backgroundColor: Colors.red),
                );
                return;
              }

              final exito = await Provider.of<BrigadaProvider>(context, listen: false).registrarVacunador(
              ...
              if (mounted) {
                ...
                _telefonoCtrl.clear(); _correoCtrl.clear();
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(exito ? 'Vacunador asignado con éxito a tu sector.' : 'Error al registrar')),
                );
              }
            },
      ...
      body: brigadaProvider.isLoading
          ...
          : Padding(
              ...
              child: Column(
                ...
                children: [
                  // Tarjeta Informativa del Sector
                  Card(
                    color: Colors.teal.shade50,
                    child: ListTile(
                      leading: const Icon(Icons.streetview, color: Colors.teal, size: 40),
                      title: const Text('Sector bajo tu supervisión:', style: TextStyle(fontWeight: FontWeight.bold)),
                      subtitle: Text(
                        sector != null ? sector['nombre'].toString().toUpperCase() : 'Buscando sector...',
                  ...
                  Row(
                    ...
                    children: [
                      const Text('Vacunadores a cargo:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      if (sector != null)
                        ElevatedButton.icon(
                          onPressed: () => _mostrarDialogoVacunador(authProvider.usuarioActual!.id, sector['id']),
                  ...
                  // Listado de Vacunadores
                  Expanded(
                    child: brigadaProvider.vacunadores.isEmpty
                        ? const Center(child: Text('No tienes vacunadores registrados todavía.'))
                        : ListView.builder(
                            itemCount: brigadaProvider.vacunadores.length,
                            itemBuilder: (context, index) {
                              final vacunador = brigadaProvider.vacunadores[index];
                              return Card(
                                child: ListTile(
                                  leading: const CircleAvatar(backgroundColor: Colors.teal, child: Icon(Icons.badge, color: Colors.white)),
                                  title: Text(vacunador.nombreCompleto),
                                  subtitle: Text('Cédula: ${vacunador.cedula} | Correo: ${vacunador.correo}'),
                                ),
                              );
                            },
                          ),
